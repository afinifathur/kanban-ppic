...
@section('top_bar')
    <div class="flex items-center justify-between w-full">
        <div>
            <h1 class="text-lg font-bold text-slate-800 leading-tight">Hasil Rangkai</h1>
            <p class="text-gray-500 text-[10px]">Hasil rangkaian yang sudah dibuat dan sedang dikelola (Work Orders)</p>
        </div>
    </div>
@endsection
...
@section('content')
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <form method="GET" action="{{ route('lost-wax.assemblies.work-orders.index') }}" class="flex flex-wrap items-end gap-3.5">
                <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request('search') || request('code') || request('customer') || request('size'))
                        <a href="{{ route('lost-wax.assemblies.work-orders.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold py-2 px-4 rounded-lg transition-all border border-slate-200 text-center">
                            Reset
                        </a>
                    @endif
                </div>
